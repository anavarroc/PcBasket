...
class OrderByNumberAscStrategy implements PlayerCollectionOrderStrategyInterface
{
    public function execute(PlayerCollection $players): PlayerCollection
    {
        $orderedPlayers = new PlayerCollection();
        $orderedPlayers->addArray($this->sortByRating($players));
    ...
    public function sortByRating(PlayerCollection $players): array
    {
        ...
        usort($playersArray, fn ($a, $b) => $a->getNumber() > $b->getNumber());
